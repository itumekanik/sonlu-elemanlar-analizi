...
class ElementFrame2D:
    def __init__(elm, id, conn, EA, EI, qx=0, qy=0, mz=0):
        elm.id = id
    ...
    def q_Local(elm):  # Lokal yayılı yük vektörü
        qx, qy, mz = elm.q
        nx, ny, L = elm.nx_ny_L()
        return [0.5*qx*L, 0.5*qy*L-mz, qy*L**2/12, 0.5*qx*L, 0.5*qy*L+mz, -qy*L**2/12]
...
EA, EI = 4200000, 87500
ElementFrame2D(id=1, conn=[1, 3], EA=EA, EI=EI)
ElementFrame2D(id=2, conn=[2, 4], EA=EA, EI=EI, qy=20)
ElementFrame2D(id=3, conn=[3, 4], EA=EA, EI=EI, mz=10)
ElementFrame2D(id=4, conn=[4, 5], EA=EA, EI=EI, qy=-30)
...
for id, node in nodes.items():
    ...
    print(f"Node {id}: [ux:{ux:.6f}, uy:{uy:.6f}, tz:{tz:.6f}] [px:{px:.2f}, py:{py:.2f}, mz:{mz:.2f}]")
...
def draw(nodes, elements, filename, US=None, factor=1):
  ...
  lc0 = mc.LineCollection(lines_mesh, linewidths=1.0, color="gray")
