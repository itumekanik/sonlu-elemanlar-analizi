...
Lx, Ly =5.0, 0.8  # Yatay ve düşey boyutlar [metre]
Nx, Ny = 4, 2  # Yatay ve düşey ağ bölme sayıları
dLx, dLy = Lx / Nx, Ly / Ny  # Yatay ve düşey boyut artımları
E = 70e6  # Elastisite modülü [kN/m^2]
p = 0.33  # Poisson oranı
h = 0.3   # Levha kalınlığı [m]
ro = 24 / 9.81  # Birim hacim kütlesi [gram/cm^3]=[ton/m^3]
g = 9.81  # m/sn^2
Px_tekil = 20  # Tekil-yük [kN]
Py_tekil = -50 # Tekil-yük [kN]
...
nodes[10].force = [Px_tekil, 0]  # İlgili noda tekil-kuvvet tanımlanıyor
nodes[13].force = [0, Py_tekil]  # İlgili noda tekil-kuvvet tanımlanıyor
...
for id, elm in elements.items():
    ...
    if id in [8]:  # 8 nolu elemanın q2y leri (dış-sınır-yük)
        elm.boundaryForceY = [0, -32, 0, 0]
...
for id, node in nodes.items():
    ux, uy = US[node.code]
    px, py = PS[node.code]
    print(f"Node {id}: [ux:{ux:.5f}, uy:{uy:.5f}] [px:{px:.2f}, py:{py:.2f}]")
